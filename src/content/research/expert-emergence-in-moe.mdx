...
subtitle: "Documenting the journey of training a mixture-of-experts model to study expert specialization emergence."
date: 2026-02-01
authors:
  - name: "sumitdotml"
...
Training a small MoE on code/math/prose to watch expert specialization emerge. GPT-2 base, 8 experts with top-1 routing in last 4 layers, warm-started from pretrained MLPs. Key findings so far: token imbalance across domains (code yields 1.9x more tokens than prose), dual entropy metrics matter for analysis, and verification tests caught several bugs before training. Currently in the training infrastructure phase.
...
This isn't a research on SOTA performance; I simply want an end-to-end MoE project ending in a trained sparse Transformer model with visualizations showing routing patterns over training, entropy curves, and per-domain expert affinity heatmaps. So, the output is a technical report documenting what happens; i.e., what emerges, what fails, what surprises.
...
In a standard transformer, every token passes through the same feed-forward network (FFN) (used interchangeably with MLP) in each layer. The FFN applies the same weights to every token, whether that token is Python syntax or a word in an essay.

MoE is where things change a little. Instead of one FFN, there are multiple FFNs called "experts," and a router network decides which expert(s) handle each token. The key insight is _sparse activation_: only a subset of experts process each token, so the model gets more total capacity without proportionally more compute.
...
- Noisy Top-k Gating <Cite id="shazeer2017" authors="Shazeer et al." year={2017} /> introduced the sparsely-gated mixture-of-experts layer. The key innovation was noisy gating: adding noise to router logits during training encourages exploration and prevents collapse to a single expert.

- Switch Transformer <Cite id="fedus2021" authors="Fedus et al." year={2021} /> simplified MoE routing to top-1, where each token goes to exactly one expert. Earlier MoE work used top-2 or soft routing, but Switch showed that top-1 works fine at scale and is simpler to implement. Their key contribution was demonstrating that sparse routing doesn't hurt quality if load balancing is handled properly.

- ST-MoE <Cite id="zoph2022" authors="Zoph et al." year={2022} /> introduced z-loss for router stability. They found that router logits can drift to extreme values during training, causing numerical instability. Their fix (penalizing large logits via z-loss) became standard practice.

- Mixtral of Experts <Cite id="jiang2024" authors="Jiang et al." year={2024} /> provided architecture reference and the expert dispatch pattern I adapted for my implementation.

### 2.3 The Three Losses

MoE training uses three loss terms:

**Language Modeling Loss (Cross-Entropy)**: This is the standard next-token prediction loss. Given tokens, predict the next one. This is the main training signal.

**Load Balancing Loss**: Without intervention, routers tend toward collapse: one expert handles everything, others go unused. Load balancing loss penalizes this imbalance.
...
This works because if expert 1 is overloaded (high `f_1`), the loss penalizes high `P_1`. The gradient flows through `P_i` (which is differentiable) and pushes the router to reduce expert 1's probability. Eventually fewer tokens get routed there.
...
**Z-Loss (Router Stability):** Router logits can drift to extreme values during training:
...
$$
\mathcal{L}_{\text{total}} = \mathcal{L}_{\text{LM}} + \alpha \cdot \mathcal{L}_{\text{balance}} + \beta \cdot \mathcal{L}_{\text{z}}
$$

where typical values are $\alpha = 0.01$ and $\beta = 0.001$.

In code:

```python
total_loss = lm_loss + balance_loss_coef * balance_loss + z_loss_coef * z_loss
# typical values: balance_loss_coef = 0.01, z_loss_coef = 0.001
```
...
### 3.1 Why I Chose GPT-2 Small

I considered the following options:

1. GPT-2 Small (124M) (Radford et al., 2019) <Cite id="radford2019" authors="Radford et al." year={2019} />: OpenAI's 2019 model, well-documented
...
How expert weights should be initialized was a difficult decision for me. It was likely to have the most dramatic impact on whether the approach would work at all.
...
### 4.3 Decision: deepcopy + Tiny Noise
...
### 4.5 The Critical Noise Formula

The noise formula matters: `param.std() * 1e-3`.

**Why `param.std()` and not `param.norm()`?**
...
def forward(self, x: Tensor) -> RouterOutput:
    ...
    # cleaning probabilities for entropy logging (before noise)
    ...
    # adding noise during training (if annealing is active)
...
### 5.2 Noisy Routing: Why I Added Noise
...
**The Straight-Through Estimator (STE) solves this:** <Cite id="bengio2013" authors="Bengio et al." year={2013} />
...
hard = torch.ones_like(soft)  # what I want in forward
...
**How `hard + (soft - soft.detach())` works**
...
### 6.2 The 10-Test Verification Suite
...
### 7.1 Dataset Selection Journey

**Code: CodeParrot-clean**

For code, I chose CodeParrot-clean <Cite id="codeparrot" authors="CodeParrot" year={2022} />: diverse Python repositories, accessible via HuggingFace streaming.
...
So I pivoted to loading MathQA <Cite id="amini2019" authors="Amini et al." year={2019} /> directly from the source ZIP file.
...
During my first attempt, I was trying WikiText-103. Rejected for being too encyclopedic. Wiki-style prose is formal, structured, heavy on facts. Not representative of natural web text.

Second attempt was me mostly comparing alternatives. I generated 100 random samples from both C4 (`allenai/c4`) and FineWeb (`HuggingFaceFW/fineweb`). Read through them. Compared style, variety, naturalness.

C4 showed more natural, conversational prose: blog posts, how-to articles, forum discussions. FineWeb was higher quality but felt more curated, less varied. OpenWebText was also considered but rejected because news/article style overlaps with code documentation and README files.

**Decision: AllenAI C4 (en).** <Cite id="raffel2020" authors="Raffel et al." year={2020} /> Most natural-sounding. Well-filtered (C4 already removes duplicates, boilerplate, etc.). Streams efficiently despite being 750GB total.

### 7.2 The MathQA Format Decision
...
### 7.3 Sequence Packing Implementation
...
**The fix**: I added a `--balance-tokens` argument to the training script.
...
### 8.1 The Train/Eval Split Decision
...
An issue I dealt with was the block-level split problem. As a naive approach, I was packing all texts into blocks, then splitting blocks into train/eval.

This caused data leakage. A single document can span multiple blocks. If blocks 47 and 48 both contain parts of the same Python file, and block 47 is in train while block 48 is in eval, the model has "seen" part of the eval data during training.

The solution was to split at the text level before packing.
...
This, I believe, prevents document leakage. The eval signal should now be meaningful.

The holdout formula that I used:
...
### 8.2 Shuffle-Before-Truncate

When token balancing truncates larger domains, naive slicing (`blocks[:min_blocks]`) systematically discards the tail.

If there's any systematic pattern in text ordering (longer texts at the end, different topics at the end), truncation could introduce bias.
...
The findings were minimal difference. The text-level shuffle in `split_texts_for_eval()` already randomizes ordering before packing.

I decided to shuffle blocks before truncating anyway.
...
### 9.2 Example Where They Diverge

Let me consider a batch where every token routes to exactly one expert with probability 1.0, but different tokens pick different experts uniformly.
...
### 11.1 Empirical Testing Matters
...
### 11.2 Token Imbalance Was a Surprise
...
### 11.3 Small Budget Means I Had to Prioritize

$80 meant I had to prioritize with some noticeable compromises. Dense baseline and MoE main run were non-negotiable. No-LB ablation demonstrated collapse. Top-2 was a stretch goal. The constraint was clarifying.

### 11.5 What I'd Do Differently

I should have started with verification tests earlier. I wrote them for Phase 2 but should have written them for Phase 1. Testing `compute_load_balance_loss` in isolation would have caught the theoretical minimum = 1.0 misunderstanding earlier.
...
If Phase 5 reveals issues (training instability, unclear results), the ablations would get cut before the main run does.

## Acknowledgments

Claude Opus 4.5 (primary) and GPT-5.2 (sparsely) have been used in this project for research assistance,
studying MoE concepts, and quick code verifications (dataset, naive tests, etc.).

## Third-Party Notices

This project uses third-party datasets and libraries. See <a href="https://github.com/sumitdotml/moe-emergence/blob/main/THIRD-PARTY-NOTICES.md" target="_blank">third party notices</a> for full attribution and license information.
...
<Ref
	id="shazeer2017"
	authors="Shazeer, N., Azalia, M., Davis, C., Le, Q., Hinton, G., and Dean, J."
	year={2017}
...
<Ref
	id="jiang2024"
	authors="Jiang, A. Q., Sablayrolles, A., Roux, A., Mensch, A., Savary, B., Bamford, C., Chaplot, D. S., de las Casas, D., Hanna, E. B., Bressand, F., Lengyel, G., Bour, G., Lample, G., Lavaud, L. R., Saulnier, L., Lachaux, M.-A., Stock, P., Subramanian, S., Yang, S., Antoniak, S., Le Scao, T., Gerber, T., Vart, T., Wang, X., and El Sayed, W."
	year={2024}
...
<Ref
	id="amini2019"
	authors="Amini, A., Gabriel, S., Lin, S., Koncel-Kedziorski, R., Choi, Y., and Hajishirzi, H."
	year={2019}
...
<Ref
	id="raffel2020"
	authors="Raffel, C., Shazeer, N., Roberts, A., Lee, K., Narang, S., Matena, M., Zhou, Y., Li, W., and Liu, P. J."
	year={2020}
...
<Ref
	...
	authors="CodeParrot"
	year={2022}
	title="codeparrot-clean dataset"
