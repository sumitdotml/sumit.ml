...
> WIP: This is the first draft of the worklog. It is not yet formatted properly.
...
The mean gap was `0.0045489006823352`, or about `0.455` percentage points, in favor of attention-only. Under the frozen rule, that is below the `0.01` threshold for a winner claim. So the honest reading is directional: attention-only ended higher in this run set, but the result stays inside the pre-declared inconclusive band.
...
If I were doing the next version, I would keep the same discipline: narrow the question first, freeze contracts before seeing benchmark numbers, treat dataset audits as part of the experiment, and make every convenience change prove that it does not change the comparison.
